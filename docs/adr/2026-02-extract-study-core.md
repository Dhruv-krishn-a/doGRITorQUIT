...
## Decision
We will extract the platform-agnostic business logic and data orchestration into a new package: `@planner/study-core`.
We will also extract presentational UI components into `@planner/study-ui-web`.

### Architecture
- **@planner/study-core**: Contains hooks, API clients, and pure logic. No UI dependencies (React is allowed for hooks).
- **@planner/study-ui-web**: Contains presentational components. Reusable by Desktop.
