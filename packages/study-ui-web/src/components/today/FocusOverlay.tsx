// packages/study-ui-web/src/components/today/FocusOverlay.tsx
"use client";
...
export function FocusOverlay({ item, onClose, onComplete }: FocusOverlayProps) {
  ...
  const content = (
    <div 
      style={{ zIndex: 99999 }}
      className="fixed inset-0 flex items-center justify-center p-4 md:p-10 pointer-events-none"
    >
      ...
      >
        ...
        <div className={`p-8 pb-4 flex justify-between items-start ${isZenMode ? 'opacity-20 hover:opacity-100 transition-opacity' : ''}`}>
          <div className="flex gap-4 items-center">
            ...
            <div>
              <div className={`text-[10px] font-black uppercase tracking-[0.2em] ${isZenMode ? 'text-rose-400' : 'text-rose-500'}`}>{item.vectorName}</div>
              <h2 className={`text-xl font-black tracking-tight leading-tight ${isZenMode ? 'text-white' : 'text-slate-900'}`}>{item.title}</h2>
            </div>
          </div>
        ...
        <div className={`flex-1 overflow-y-auto p-8 pt-4 flex gap-8 ${(isExpanded || isZenMode) ? 'flex-row' : 'flex-col'}`}>
          <div className="flex-1 flex flex-col items-center justify-center gap-10 py-8">
            <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-6xl font-black tabular-nums tracking-tighter ${isZenMode ? 'text-white' : 'text-slate-900'}`}>{formatTime(seconds)}</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest mt-3 ${isZenMode ? 'text-rose-400' : 'text-slate-400'}`}>Neural Investment</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-6">
                ...
                {!isZenMode && (
                    <button 
                        ...
                        className="px-10 py-6 bg-slate-900 text-white rounded-full font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-slate-200 hover:bg-emerald-500 hover:shadow-emerald-200 transition-all active:scale-95 flex items-center gap-4 whitespace-nowrap min-w-fit"
                    >
          ...
          {!isZenMode && (
            <div className={`${isExpanded ? 'w-96' : 'w-full'} flex flex-col gap-4`}>
                ...
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Capture insights..." className="w-full flex-1 min-h-[250px] bg-slate-50 border border-slate-100 rounded-[2.5rem] p-8 text-sm font-bold text-slate-800 focus:bg-white focus:border-rose-200 focus:ring-8 focus:ring-rose-50/50 outline-none transition-all placeholder:text-slate-300 resize-none shadow-inner" />
            </div>
          )}
        </div>
        ...
        <div className={`p-8 py-6 border-t flex items-center justify-between ${isZenMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50/50 border-slate-100'}`}>
           ...
           {isZenMode && (
               <button onClick={() => onComplete(item.id, item.type, seconds)} className="text-rose-400 font-black text-[10px] uppercase tracking-widest hover:text-rose-300 transition-colors">
                   Finish & Exit Zen Mode
               </button>
           )}
           <div className={`text-[10px] font-black uppercase tracking-widest ${isZenMode ? 'text-slate-700' : 'text-slate-400'}`}>
