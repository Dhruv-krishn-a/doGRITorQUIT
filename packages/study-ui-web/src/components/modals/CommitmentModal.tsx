...
export function CommitmentModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        <div className="p-8 md:p-10 flex flex-col h-full max-h-[90vh]">
          <header className="flex justify-between items-start mb-8">
            <div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Daily Goal</h2>
              <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Plan your course completion</p>
            </div>
            <button onClick={closeModal} className="p-2 text-slate-400 hover:text-rose-600 transition-all">
              <X size={24} />
            </button>
          </header>
          ...
          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 mb-8 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Course Duration</p>
                <p className="text-lg font-black text-slate-900">{formatDuration(totalMinutes)}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lessons</p>
              <p className="text-lg font-black text-slate-900">{activeTrack.track.units?.length || 0}</p>
            </div>
          </div>
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-8">
            <button 
              ...
              className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'DATE' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Finish by Date
            </button>
            <button 
              onClick={() => setMode('TIME')}
              className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'TIME' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Study Time / Day
            </button>
          </div>

          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar space-y-8 pr-2">
               ...
               {mode === 'DATE' ? (
                 ...
                 >
                   <div className="space-y-4">
                     <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">When do you want to finish?</label>
                     <div className="relative">
                       <Calendar className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" size={20} />
                       <input 
                         type="date" 
                         required
                         className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-16 py-5 font-bold text-slate-800 focus:border-rose-500 focus:outline-none transition-all"
                         value={targetDate}
                 ...
                 >
                   <div className="space-y-6">
                     <div className="flex items-center justify-between">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Study time per day</label>
                        <span className="text-2xl font-black text-rose-600">{formatDuration(dailyStudyMinutes)}</span>
                     </div>
                     <input 
                       type="range" 
                       min="10" 
             ...
             {analysis && !analysis.error && (
               <motion.div 
                 ...
                 className="bg-rose-50/50 p-8 rounded-[2rem] border border-rose-100 space-y-6"
               >
                 <div className="grid grid-cols-2 gap-8">
                   <div>
                     ...
                     <p className="text-2xl font-black text-slate-900">{analysis.formattedWatch}<span className="text-xs text-slate-400 ml-1">/ day</span></p>
                   </div>
                   <div>
                     {mode === 'DATE' ? (
                       <>
                         ...
                         <p className="text-2xl font-black text-slate-900">{analysis.formattedStudy}<span className="text-xs text-slate-400 ml-1">/ day</span></p>
                       </>
                     ) : (
                       <>
                         <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                         ...
                         <p className="text-2xl font-black text-slate-900">{analysis.estDate}</p>
                       </>
                     )}
                   </div>
                 </div>

                 <div className="pt-6 border-t border-rose-100 flex gap-3 items-start">
             ...
             {analysis?.error && (
               <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100 flex gap-3 items-center text-amber-700">
                 <AlertCircle size={20} />
             ...
             <button 
               ...
               type="submit" 
               className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-rose-600 shadow-xl shadow-slate-200 transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:bg-slate-200 disabled:shadow-none"
             >
