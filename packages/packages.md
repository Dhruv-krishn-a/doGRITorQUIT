...
- **Consumption**: Applications in `apps/` (web, mobile, cms, desktop) import these packages using their internal names (e.g., `@gritorquit/db`, `@gritorquit/domain`).
- **Dependencies**: Packages often depend on each other. For example, `@gritorquit/domain` depends on `@gritorquit/db` for data access, and `@gritorquit/api` is used by all frontend apps for input validation.
