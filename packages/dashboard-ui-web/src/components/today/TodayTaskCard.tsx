// packages/dashboard-ui-web/src/components/today/TodayTaskCard.tsx
"use client";
...
export function TodayTaskCard({ 
  ...
  if (TaskCardComponent) {
    const CustomTaskCard = TaskCardComponent as any;
    return (
      <CustomTaskCard 
        task={item} 
        isCompact={true}
  ...
  return (
    <div className="transform-gpu p-3 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl text-sm font-semibold text-[var(--text-primary)]">
      {item.title}
    </div>
  );
}
