...
export const StudyService = {
    ...
    const unitsData = playlistItems.map((v) => {
      ...
      return {
        ...
        metadata: {
          ...
          playlistIndex: v.position // Core numbering persistent field
  ...
  async syncPlaylist(userId: string, trackId: string) {
    ...
    // 1. Identify existing and new videos
    const existingUnits = track.units;
    const existingVideoIds = new Set(existingUnits.map(u => (u.metadata as any)?.youtubeId).filter(Boolean));
    const newItems = playlistItems.filter(item => !existingVideoIds.has(item.id));

    // 2. Update orderIndex and metadata for ALL existing videos based on current playlist position
    for (const item of playlistItems) {
      const existingUnit = existingUnits.find(u => (u.metadata as any)?.youtubeId === item.id);
      if (existingUnit) {
        const meta = (existingUnit.metadata as any) || {};
        await prisma.unit.update({
          where: { id: existingUnit.id },
          data: { 
            orderIndex: item.position,
            metadata: { ...meta, playlistIndex: item.position }
          }
        });
      }
    }
    ...
    // 3. Enforce limit and add new items
    const maxVideos = await getFeatureLimit(userId, PlanFeature.MAX_VIDEOS_PER_PLAYLIST, 50, 500);
