...
- **`react-library.json`**: A configuration for React components used within internal packages (e.g., `@gritorquit/ui`).
- **`package.json`**: Defines the `@gritorquit/typescript-config` package and its dependencies.
