...
def update_web():
  ...
  watchPercentage
    ...
    # 4. Update StudyView state and effects
  ...
  const [currentTab, setCurrentTab] = useState<"NOTES" | "QUESTIONS">("NOTES");'''
    ...
    # 5. Update handleSaveNotes
    content = content.replace(
        'await saveNotes(unitId as string, notes);',
        'await saveNotes(unitId as string, { freeform: freeformNotes, questions });'
    )

    # 6. Update NotesPanel instances
    content = content.replace('notes={notes}', 'freeformNotes={freeformNotes}\n                  setFreeformNotes={setFreeformNotes}\n                  questions={questions}\n                  setQuestions={setQuestions}')
    content = content.replace('setNotes={setNotes}', '') # Handled in above replacement
    content = content.replace('currentCategory={currentCategory}', 'currentTab={currentTab}')
    content = content.replace('setCurrentCategory={setCurrentCategory}', 'setCurrentTab={setCurrentTab}')
