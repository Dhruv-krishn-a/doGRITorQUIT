...
export default function CMSLoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#fdfbfb] relative overflow-hidden font-sans">
       ...
       <div className="relative z-10 w-full max-w-md p-4 animate-in fade-in slide-in-from-bottom-4 duration-1000">
          <div className="bg-white/60 backdrop-blur-2xl p-10 rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.1)] border border-white overflow-hidden">
            
            <div className="flex flex-col items-center mb-10 text-center">
              <div className="w-16 h-16 bg-linear-to-br from-rose-500 to-fuchsia-500 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-rose-200 mb-6">
                <Sparkles size={32} />
              </div>
            ...
            {error && (
              <div className="bg-rose-50 text-rose-600 p-4 rounded-2xl mb-6 text-xs font-bold uppercase tracking-widest border border-rose-100 animate-shake">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-4">Identity@Core.Sys</label>
                <input 
                  type="email" 
                  required
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full bg-white/80 border border-slate-200 rounded-[1.5rem] px-6 py-4 font-bold text-slate-800 placeholder:text-slate-300 focus:border-rose-400 focus:ring-4 focus:ring-rose-100/50 outline-none transition-all shadow-inner text-sm"
                  placeholder="admin@gritorquit.com"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="mt-10 text-center pt-8 border-t border-rose-50">
