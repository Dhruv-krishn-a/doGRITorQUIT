...
- **Internal Packages**: The CMS app imports shared packages such as `@gritorquit/db` for database management and `@gritorquit/config` for system-wide settings.
...
- **Consistency**: Centralized navigation and common UI components (where possible) from `@gritorquit/ui` ensure a consistent experience for administrators.
