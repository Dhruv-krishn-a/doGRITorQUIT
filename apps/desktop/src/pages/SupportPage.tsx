...
export default function SupportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/support'), {
        method: 'POST',
      ...
      if (!res.ok) {
        console.error("Direct send failed, falling back to mailto:", await res.text());
        const subject = encodeURIComponent("Support Request: Desktop App");
  ...
  return (
    <div className="flex flex-col h-screen bg-[var(--bg-primary)] overflow-hidden">
      <TitleBar />
      <div className="flex-1 transform-gpu relative overflow-hidden font-sans text-[var(--text-primary)] flex items-center justify-center">
        ...
        <div className="transform-gpu w-full max-w-4xl mx-auto p-4 relative z-10">
          ...
          >
            {/* Left Panel: Info */}
            <div className="transform-gpu lg:col-span-2 p-12 bg-gradient-to-br from-[var(--accent-color)]/20 to-transparent border-r border-[var(--border-color)] flex flex-col justify-between">
              <Link to="/login" className="transform-gpu flex items-center gap-3 group w-fit">
              ...
              <AnimatePresence mode="wait">
                {success ? (
                  ...
                  >
                    ...
                    <Link to="/login" className="px-8 py-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[var(--hover-bg)] transition-all">
                      Return to Sign In
                    </Link>
                  </motion.div>
                ) : (
