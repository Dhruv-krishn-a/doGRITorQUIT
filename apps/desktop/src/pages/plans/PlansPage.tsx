...
export default function PlansPage() {
 ...
 const handleDeletePlan = async (planId: string) => {
  ...
  try {
    ...
    await fetch(buildApiUrl(`/plans/${planId}`), {
      method: 'PATCH',
      headers: {
 ...
 if (loading) return <div className="p-8 flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-slate-900"></div></div>;
  ...
  <div className="p-4 md:p-8 max-w-7xl mx-auto w-full">
    ...
    {plans.length > 0 ? (
     ...
     <div className="flex flex-col items-center justify-center py-20 bg-white/50 border-2 border-dashed border-slate-200 rounded-3xl text-center">
       <h3 className="text-2xl font-bold text-slate-900 mb-2">No roadmaps yet</h3>
