...
export default function TrackDetailPage() {
  ...
  if (loading && !activeTrack) return <div className="p-20 text-center text-rose-500 font-black animate-pulse uppercase tracking-widest">Loading course...</div>;
  ...
  const MainBoard = (
    <div className="relative space-y-10 text-slate-900 pb-20 w-full max-w-full overflow-y-auto h-full px-8 pt-8">
      {/* Background Accent */}
      <div className="fixed top-0 right-0 w-125 h-125 bg-rose-50/50 rounded-full blur-[120px] -z-10 pointer-events-none opacity-50" />
        ...
        <div className="flex items-center gap-6 w-full">
          ...
          <div className="flex-1 min-w-0">
             <div className="flex items-center gap-3 mb-1">
               <h1 className="text-3xl font-black text-slate-900 tracking-tight truncate">{activeTrack.track.title}</h1>
               <span className="text-[10px] font-black bg-slate-900 text-white px-2 py-0.5 rounded-full uppercase tracking-widest shadow-lg shadow-rose-200 shrink-0">{activeTrack.track.type === 'PLAYLIST' ? 'Course' : 'Manual'}</span>
             </div>
      ...
      <main className="animate-in fade-in slide-in-from-bottom-4 duration-1000 w-full">
    ...
    <div className="flex flex-col h-full">
      ...
      {selectedUnit ? (
        <div className="flex-1 p-6 overflow-y-auto">
          <div className="mb-6">
            <span className="text-[9px] font-black bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg uppercase tracking-widest border border-indigo-100">
              {selectedUnit.type} Context
            </span>
            <h1 className="text-2xl font-black text-slate-900 mt-4 mb-2 leading-tight">
              {selectedUnit.title}
            </h1>
        ...
        <div className="flex-1 p-8 flex flex-col items-center justify-center text-center text-slate-400 bg-slate-50/30">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest mb-2">No Active Context</p>
