...
import { Github, GitBranch, GitCommit, GitPullRequest, Loader2, Link2, ExternalLink } from "lucide-react";
...
export function GithubEvidencePanel({ projectId, feature, repoName, onUpdateEvidence }: GithubEvidencePanelProps) {
    ...
    try {
      if (feature.githubBranch) {
        const [commitsRes, prsRes] = await Promise.all([
          api.get(`/api/github-projects/${projectId}/repo-data?action=commits&branch=${feature.githubBranch}`),
          api.get(`/api/github-projects/${projectId}/repo-data?action=prs`)
        ]);
  ...
  const loadBranchesForLinking = async () => {
    ...
    try {
      const data = await api.get(`/api/github-projects/${projectId}/repo-data?action=branches`);
      setBranches(data || []);
    } catch (err: any) {
        ...
        {feature.githubBranch && (
          <div className="space-y-3">
            ...
            {prs.length > 0 ? (
               ...
               <div className="text-[10px] font-bold text-[var(--text-secondary)] italic uppercase tracking-widest opacity-60">No pull requests linked.</div>
            )}
          </div>
        )}
