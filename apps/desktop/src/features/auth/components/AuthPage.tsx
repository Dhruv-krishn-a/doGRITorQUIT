...
import { 
  Mail, Lock, ArrowRight, Loader2, Sparkles, AlertCircle, CheckCircle2, ArrowLeft, ShieldCheck, Phone, KeyRound, Github
} from "lucide-react";
...
interface AuthPageProps {
  view: "login" | "signup" | "forgot-password" | "update-password";
}
...
export default function AuthPage({ view: initialView }: AuthPageProps) {
  ...
  useEffect(() => {
    if (view !== "update-password") {
      if (authService.getSession()) navigate("/");
    }
    ...
    const handleOAuthError = (e: any) => {
      setError(e.detail || "OAuth Authentication failed");
      setLoading(false);
    };
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (view === "signup") {
        const res = await fetch(buildApiUrl('/native-auth/credentials/register'), {
          method: 'POST',
        ...
        setSuccessMsg(payload.message || "Account created. Verify your email before signing in.");
        setTimeout(() => setView("login"), 1800);
      } else if (view === "login") {
        const res = await fetch(buildApiUrl('/native-auth/login'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email, password })
        });
        ...
        if (!res.ok) {
          ...
          if (payload?.error === "legacy_account") {
            throw new Error("Legacy account detected. Reset password to continue.");
          }
          throw new Error(payload?.error || "Login failed");
        }
        authService.setSession({ access_token: payload.token, user: payload.user });
        navigate("/");
      } else if (view === "forgot-password") {
        const res = await fetch(buildApiUrl('/native-auth/password/forgot'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const resetToken = params.get("token") || params.get("native_token") || params.get("access_token");
        
        const res = await fetch(buildApiUrl('/native-auth/password/set'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("Phone OTP flow is not available yet. Use email or OAuth sign in.");
  };

  const handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("Phone OTP verification is not available yet. Use email or OAuth sign in.");
  };
    ...
    <div className="transform-gpu min-h-screen w-full flex items-center justify-center bg-[var(--bg-primary)] relative overflow-hidden font-sans text-[var(--text-primary)] transition-colors duration-500">
      ...
      <div className="transform-gpu w-full max-w-5xl mx-auto p-4 relative z-10">
        ...
        >
          ...
          <div className="transform-gpu hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-[var(--accent-color)]/20 to-transparent relative overflow-hidden border-r border-[var(--border-color)]">
             <div className="transform-gpu absolute top-[-10%] right-[-10%] w-64 h-64 bg-[var(--accent-color)]/10 rounded-full blur-3xl" />
             
             <Link to="/" className="transform-gpu relative z-10 flex items-center gap-3 group w-fit">
          ...
          <div className="transform-gpu flex flex-col justify-center p-8 md:p-12 w-full max-w-md mx-auto relative">
            ...
            {authMode === 'email' ? (
              <form onSubmit={handleEmailAuth} className="transform-gpu space-y-5">
                {view !== 'update-password' && (
                ...
                {view === 'login' && (
                  <motion.div variants={itemVariants} className="transform-gpu flex justify-between items-center">
                    <button 
                      type="button"
                      onClick={() => setAuthMode('phone')}
                      className="transform-gpu text-[10px] font-black text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors uppercase tracking-widest flex items-center gap-1"
                    >
                      <Phone size={12} /> Phone Login
                    </button>
                    <button 
                      type="button"
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                className="transform-gpu space-y-5"
              >
                {!showOtp ? (
                  <form onSubmit={handlePhoneSubmit} className="space-y-5">
                    <motion.div variants={itemVariants} className="transform-gpu space-y-2">
                      <label className="transform-gpu text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em] ml-1">Phone Number</label>
                      <div className="transform-gpu relative group">
                        <Phone size={18} className="transform-gpu absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-[var(--accent-color)] transition-colors" />
                        <input 
                          type="tel" 
                          value={phone}
                  ...
                  <form onSubmit={handleOtpSubmit} className="space-y-5">
                    <motion.div variants={itemVariants} className="transform-gpu space-y-2">
                      <label className="transform-gpu text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em] ml-1">Verification Code</label>
                      <div className="transform-gpu relative group">
                        <KeyRound size={18} className="transform-gpu absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-[var(--accent-color)] transition-colors" />
                        <input 
                          type="text" 
                          value={otp}
                ...
                <motion.div variants={itemVariants} className="transform-gpu mt-4 text-center">
                  <button 
                    type="button"
                    onClick={() => { setAuthMode('email'); setShowOtp(false); }}
                    className="transform-gpu inline-flex items-center gap-2 text-[10px] font-black text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors uppercase tracking-widest"
                  >
                    <ArrowLeft size={14} /> Use Email Address
                  </button>
                </motion.div>
              </motion.div>
            )}
              ...
              <>
                ...
                <motion.div variants={itemVariants} className="mt-3">
                  <button
                    type="button"
                    disabled
                    className="transform-gpu w-full flex items-center justify-center gap-3 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl py-4 text-xs font-black text-[var(--text-secondary)] opacity-60 cursor-not-allowed uppercase tracking-widest"
                  >
            ...
            {(view === 'forgot-password' || view === 'update-password') && (
              <motion.div variants={itemVariants} className="transform-gpu mt-8 text-center">
                <button 
                  onClick={() => {
                    if (view === 'update-password') navigate('/login');
                    else setView('login');
                  }}
