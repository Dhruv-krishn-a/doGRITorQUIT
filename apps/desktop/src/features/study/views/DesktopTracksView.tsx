...
export function DesktopTracksView() {
  ...
  const handleDeletePlan = async (planId: string) => {
    ...
    try {
        ...
        // It's better to add deletePlan to usePlans hook! Let's do that or just fetch.
        // The previous PlansPage had session. Let's just import useAuth.
        const authModule = await import('../../auth/hooks/useAuth');
        const session = authModule.useAuth().session;
  ...
  if (loading && !dashboard) return (
    <div className="transform-gpu flex items-center justify-center min-h-[60vh] w-full bg-[#fdfbfb]">
  ...
  return (
    <div className="transform-gpu relative w-full h-full bg-[#fdfbfb] text-slate-800 selection:bg-rose-200 selection:text-rose-900 font-sans overflow-y-auto overflow-x-hidden">
          ...
          <div className="transform-gpu space-y-2">
            <div className="transform-gpu flex items-center gap-3">
              <h1 className="transform-gpu text-4xl font-bold text-slate-900 tracking-tighter uppercase">
                Upgrade <span className="transform-gpu text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-fuchsia-500">OS</span>
              </h1>
                                  ...
                                  {isLimitReached ? (
                                      ...
                                      <div className="transform-gpu space-y-1">
                                          <button 
                                              ...
                                              className="transform-gpu w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-gradient-to-r from-rose-50 to-fuchsia-50 hover:from-rose-100 hover:to-fuchsia-100 transition-all text-left group border border-rose-100/50"
                                          >
                                              <div className="transform-gpu w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-fuchsia-500 text-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                                  <Sparkles size={16} />
                                              </div>
                                              <div>
