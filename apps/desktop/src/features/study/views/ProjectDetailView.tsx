...
import { useStudy, Unit, UnitStatus } from '@planner/study-core';
...
import type { ProjectTab, EnergyLevel } from '@planner/study-ui-web';
...
export function ProjectDetailView() {
  ...
  return (
    <div className="transform-gpu flex flex-col w-full h-full flex-1 bg-[#fdfbfb] text-slate-800 overflow-hidden font-sans selection:bg-rose-100 selection:text-rose-900 relative">
      {/* --- BACKGROUND GLOW --- */}
      ...
      {/* --- STICKY TOP BAR --- */}
      <div className="transform-gpu shrink-0">
        <ProjectHeader 
          {...sharedProps}
      ...
      <div className="transform-gpu flex-1 flex overflow-hidden">
        {/* --- LEFT COLUMN: SideBar --- */}
        <div className="transform-gpu shrink-0 h-full border-r border-slate-100">
          <ProjectSidebar 
            {...sharedProps}
        ...
        <main className="transform-gpu flex-1 relative overflow-hidden h-full">
            <div className="transform-gpu absolute inset-0 overflow-y-auto no-scrollbar">
                {activeTab === 'OVERVIEW' && <ProjectOverviewTab {...sharedProps} />}
                
                {activeTab === 'BOARD' && (
                  <div className="transform-gpu p-6 md:p-8">
                    <ProjectBoardTab 
                      units={units}
                      viewMode={viewMode}
        ...
        <div className="transform-gpu shrink-0 h-full border-l border-slate-100 bg-white/40 backdrop-blur-md">
          <ProjectContextPanel 
            track={track}
            units={units}
