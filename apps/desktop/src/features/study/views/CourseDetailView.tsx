...
export function CourseDetailView() {
  ...
  return (
    <div className="transform-gpu flex-1 min-w-0 relative space-y-12 text-rose-100 w-full font-sans bg-[#0a0105] flex flex-col h-full overflow-hidden">
      ...
      <div className="transform-gpu relative z-10 flex flex-col h-full w-full">
        <header className="transform-gpu flex flex-col gap-10 w-full p-6 md:p-8 shrink-0">
          <div className="transform-gpu flex flex-col md:flex-row md:items-center gap-8 w-full">
            <button 
              onClick={() => navigate('/study')} 
              className="transform-gpu p-5 bg-[#14030b] border border-rose-900/50 rounded-3xl text-rose-300/50 hover:text-rose-400 hover:border-rose-500/50 hover:bg-[#1c0510] transition-all active:scale-95 shrink-0 self-start"
            >
              <ArrowLeft size={28} />
            </button>
            ...
            <div className="transform-gpu flex flex-wrap gap-4 shrink-0">
              <button className="transform-gpu p-5 bg-[#14030b] border border-rose-900/50 rounded-3xl text-rose-300/50 hover:text-rose-400 hover:bg-[#1c0510] hover:border-rose-500/50 transition-all active:scale-95"><Share2 size={24} /></button>
              <button onClick={() => openModal('COMMIT')} className="transform-gpu p-5 bg-[#14030b] border border-rose-900/50 rounded-3xl text-rose-300/50 hover:text-rose-400 hover:bg-[#1c0510] hover:border-rose-500/50 transition-all active:scale-95"><Settings size={24} /></button>
              <button onClick={async () => { await fetchTrack(track.id); openModal('DELETE'); }} className="transform-gpu p-5 bg-[#14030b] border border-red-900/50 rounded-3xl text-red-400/60 hover:text-red-400 hover:bg-red-950/30 hover:border-red-500/50 transition-all active:scale-95"><Trash2 size={24} /></button>
            </div>
          </div>

          <div className="transform-gpu mt-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-2">
            <TrackHeader 
        ...
        <div className="transform-gpu flex-1 flex flex-col min-h-0 bg-black/20">
          <div className="transform-gpu flex items-center justify-between px-8 py-6 border-b border-rose-900/40">
          ...
          <main className="transform-gpu flex-1 relative overflow-hidden">
            <div className="transform-gpu absolute inset-0 overflow-y-auto custom-scrollbar p-8">
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'NOTES' && (
                  <motion.div key="notes" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="transform-gpu space-y-6 max-w-6xl mx-auto">
                     <div className="transform-gpu bg-[#14030b] border border-rose-900/40 rounded-[2.5rem] p-10 space-y-8">
                        <div className="transform-gpu flex items-center justify-between">
                          <div>
                            ...
                            <p className="transform-gpu text-[10px] font-bold text-rose-400/40 uppercase tracking-widest mt-1">Syllabus-wide synthesis</p>
                          </div>
                          <button 
                            onClick={async () => {
                              setIsSaving(true);
                            ...
                            className="transform-gpu flex items-center gap-2 px-8 py-4 bg-linear-to-r from-fuchsia-600 to-purple-600 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:from-fuchsia-500 hover:to-purple-500"
                          >
