...
export const useAIArchitect = (setOpen: (open: boolean) => void) => {
  ...
  const callAI = async (payload: Record<string, unknown>) => {
    const res = await fetch(buildApiUrl('/ai/plan'), {
        method: "POST",
        headers: { 
  ...
  const generateInBatches = async (syllabus: SyllabusData) => {
    ...
    const totalDays = syllabus.modules.reduce((acc: number, mod: SyllabusModule) => {
    ...
    syllabus.modules.forEach((mod: SyllabusModule) => {
        const dStr = String(mod.duration);
  ...
  const startPlan = async (startDate: string) => {
     ...
     try {
        const importPayload = {
            ...
            startDate: startDate, 
            isAI: true,
            tasks: currentBlueprint.tasks.map((t: TaskBlueprint) => ({
              "Day": t.day,
              "Task Title": t.title,
        ...
        const res = await fetch(buildApiUrl('/plans/import-json'), {
            method: "POST",
            headers: { 
                "Content-Type": "application/json",
                "Authorization": `Bearer ${session?.access_token}`
        ...
        if (res.ok) { 
           ...
           setOpen(false); 
           navigate(`/study`); // Return to study dashboard where the plan will be shown
           // Need to trigger a refresh in the parent component via callback ideally
        } else {
