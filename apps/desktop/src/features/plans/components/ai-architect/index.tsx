    ...
    {open && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
       <motion.div 
        ...
        className="bg-white w-full max-w-7xl h-[90vh] rounded-2xl flex overflow-hidden shadow-2xl border border-slate-200"
       >
        {/* --- LEFT PANEL: Chat (40%) --- */}
        {/* ✅ Fixed: Updated to canonical Tailwind class */}
        <div className="w-100 flex-none flex flex-col border-r border-slate-200 bg-slate-50/50">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {messages.length === 0 && (
              <div className="mt-10 flex flex-col items-center justify-center opacity-40">
                ...
                <h4 className="text-xl font-bold text-slate-700">What are we building?</h4>
                <p className="text-slate-500 mt-2 text-center text-sm">e.g., &quot;React in 60 days&quot;</p>
              </div>
            )}
            ...
            {loading && (
              <div className="flex gap-3 animate-pulse ml-1">
                ...
                <div className="text-sm text-slate-500 py-1.5 font-medium">
                  AI is thinking...
                </div>
              </div>
            )}
        ...
        <div className="flex-1 flex flex-col bg-white relative">
          <button 
            ...
            className="absolute top-4 right-4 z-10 p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-full"
          >
            <X size={20} />
          </button>
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-slate-50/30">
            {!currentSyllabus && !currentBlueprint && (
               <div className="h-full flex flex-col items-center justify-center text-slate-300">
                ...
                <h2 className="text-xl font-semibold text-slate-400">Preview Area</h2>
                <p className="text-sm">Generated roadmaps will appear here.</p>
               </div>
            )}

            {currentSyllabus && !currentBlueprint && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-2xl font-bold text-slate-800 mb-2">Structure Review</h2>
                <p className="text-slate-500 mb-6">Confirm the modules before generating the timeline.</p>
                <SyllabusReview 
