  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-500 mb-2 text-left">Atmospheric Scan</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Focus Time */}
        <div className="bg-slate-surface/30 p-6 rounded-[2.5rem] border border-slate-800 shadow-sm relative overflow-hidden group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-auto md:h-[500px]">
        ...
        <div className="md:col-span-3 flex flex-col gap-6 h-full">
           ...
           {/* Active Plan */}
           <div className="h-3/5 bg-slate-800/40 rounded-[3rem] border border-slate-800 p-10 flex flex-col justify-between relative overflow-hidden">
              <div className="flex justify-between items-start z-10">
                 <div>
                    <div className="flex items-center gap-2 text-sky-focus text-[10px] font-black uppercase tracking-widest mb-4 text-left">
                       <Target size={14} /> Current Mission
                    </div>
                    <h3 className="text-4xl font-black text-white italic uppercase tracking-tighter">{activePlan?.title || "No Active Mission"}</h3>
                 </div>
                 <div className="text-right">
                    <div className="text-4xl font-black text-slate-700 italic">Day {activePlan?.currentDay || 0}</div>
                    <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">of {activePlan?.totalDays || 0}</div>
                 </div>
              </div>
              ...
              {activePlan ? (
                 <div className="z-10">
                    <div className="flex justify-between items-end mb-4">
                       <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-left">Sector Progress</p>
                       <p className="text-2xl font-black text-white italic">{activePlan.progress}%</p>
                    </div>
                    <div className="w-full h-4 bg-slate-900 rounded-full overflow-hidden border border-slate-700 p-1">
                       <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${activePlan.progress}%` }}
                          transition={{ duration: 1.5, ease: "easeOut" }}
                          className="h-full bg-sky-focus rounded-full shadow-lg shadow-sky-500/20" 
                       />
                    </div>
                 </div>
              ) : (
                 <Link to="/study" className="z-10 inline-flex items-center gap-3 px-8 py-4 bg-sky-focus text-obsidian rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-lg shadow-sky-500/20 active:scale-95 w-fit">
           ...
           <div className="h-2/5 flex flex-col">
              <div className="flex items-center justify-between mb-4 ml-1">
                 ...
                 <Link to="/daily-checklist" className="text-[9px] font-black text-sky-focus uppercase tracking-widest hover:underline">Full Array &rarr;</Link>
              </div>
              <div className="grid grid-cols-3 gap-6 flex-1">
                 {habits.slice(0, 3).map(h => (
                    <div key={h.id} className="bg-slate-surface/20 rounded-[2rem] border border-slate-800 p-6 flex flex-col justify-between group hover:border-sky-focus/30 transition-all">
                       <div className="flex justify-between items-start">
                          <div className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all ${h.completedToday ? 'bg-mint border-mint' : 'bg-slate-800 border-slate-700'}`}>
                             {h.completedToday && <CheckCircle2 size={20} className="text-obsidian" />}
                          </div>
                          <span className="text-[10px] font-black text-slate-700 uppercase italic">{h.streak}X Streak</span>
                       </div>
                       <p className={`text-lg font-black uppercase italic tracking-tight truncate ${h.completedToday ? 'text-slate-600' : 'text-white'}`}>{h.title}</p>
                    </div>
                 ))}
              </div>
           </div>
        ...
        <div className="md:col-span-1 bg-slate-surface/30 rounded-[3rem] border border-slate-800 p-8 h-full flex flex-col overflow-hidden shadow-2xl relative">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3">
                 <Calendar size={20} className="text-sky-focus" /> Agenda
              </h3>
           ...
           <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
              {todaysTasks.length === 0 && (
                 <div className="p-10 border-2 border-dashed border-slate-800 rounded-[2.5rem] flex flex-col items-center justify-center text-center mt-10">
              ...
              {todaysTasks.map((task) => (
                 <div key={task.id} className="group flex gap-4 items-center p-4 bg-obsidian/50 rounded-2xl border border-slate-800 hover:border-sky-focus/30 transition-all">
                    <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${task.priority === 'HIGH' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]' : task.priority === 'MEDIUM' ? 'bg-amber' : 'bg-sky-500'}`} />
           ...
           <Link to="/today" className="mt-8 group flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-white text-obsidian text-[10px] font-black uppercase tracking-[0.2em] hover:bg-sky-focus transition-all shadow-lg active:scale-95">
