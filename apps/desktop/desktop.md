...
- **Internal Packages**: The desktop app imports shared packages such as `@gritorquit/api` and `@gritorquit/config` to ensure data and configuration consistency with other platforms.
...
- **Consistency**: Centralized configuration from `@gritorquit/config` ensures that navigation and site structures are consistent with the web and mobile platforms.
