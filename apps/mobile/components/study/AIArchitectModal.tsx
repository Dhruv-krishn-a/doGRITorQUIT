...
export const AIArchitectModal: React.FC<AIArchitectModalProps> = ({ isVisible, onClose, onGenerate }) => {
 ...
 key={t}
 onPress={() => setType(t)}
 className={`flex-1 p-6 rounded-2xl border items-center ${type === t ? 'bg-[var(--accent-color)] border-[var(--accent-color)] ' : 'bg-[var(--bg-secondary)] border border-[var(--border-color)]'}`}
 >
 ...
 className={`w-full p-6 rounded-3xl items-center ${!prompt.trim() ? 'bg-gray-500 opacity-20' : 'bg-[var(--accent-color)] '}`}
 >
