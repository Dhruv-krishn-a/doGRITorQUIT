import React, { useState, useCallback } from 'react';
import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, RefreshControl } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { useRouter, useFocusEffect } from 'expo-router';
import { useTheme } from '../../context/ThemeContext';
import { useStudyHub } from '../../hooks/useStudyHub';
import { useSync } from '../../context/SyncContext';
import { RoadmapInitModal } from '../../components/study/modals/RoadmapInitModal';

export default function RoadmapTrackerPage() {
  const { categorizedTracks, loading, refreshTracks } = useStudyHub();
  const { isSyncing, sync } = useSync();
  const router = useRouter();
  const { colors } = useTheme();
  const [creationVisible, setCreationVisible] = useState(false);

  const roadmaps = categorizedTracks.plan;

  useFocusEffect(
    useCallback(() => {
      refreshTracks();
    }, [])
  );

  const onRefresh = () => {
    sync().then(() => refreshTracks());
  };

  const RoadmapCard = ({ track }: { track: any }) => {
    return (
      <TouchableOpacity 
        onPress={() => {
          // ALWAYS navigate to the specialized Roadmap UI for study tracks
          router.push(`/study/plan/${track.id}` as any);
        }}
        activeOpacity={0.7}
        className="bg-[var(--bg-card)]/40 rounded-[2.5rem] p-8 border border-[var(--border-color)] mb-5 shadow-sm"
      >
        <View className="flex-row items-center">
          <View className="w-16 h-16 rounded-3xl items-center justify-center bg-[var(--bg-secondary)] border border-[var(--border-color)]">
            <Ionicons name="map" size={28} color="#0ea5e9" />
          </View>
          <View className="ml-5 flex-1">
            <Text className="text-[var(--text-primary)] font-black text-xl italic uppercase tracking-tighter leading-tight text-left" numberOfLines={2}>
              {track.title}
            </Text>
            <View className="flex-row items-center mt-4">
              <View className="flex-1 h-1.5 bg-[var(--bg-secondary)] rounded-full overflow-hidden border border-[var(--border-color)]/50">
                <View 
                  className="h-full rounded-full bg-[#0ea5e9] shadow-lg shadow-sky-500/50" 
                  style={{ width: `${track.progressPercentage}%` }} 
                />
              </View>
              <Text className="ml-4 text-[10px] font-black italic tracking-tighter text-sky-500">
                {Math.round(track.progressPercentage)}%
              </Text>
            </View>
          </View>
        </View>
      </TouchableOpacity>
    );
  };

  return (
    <View className="flex-1 bg-[var(--bg-primary)]">
      <ScrollView
        className="flex-1"
        contentContainerStyle={{ padding: 24, paddingTop: 60, paddingBottom: 120 }}
        refreshControl={
          <RefreshControl 
            refreshing={isSyncing} 
            onRefresh={onRefresh} 
            tintColor="#0ea5e9" 
            colors={["#0ea5e9"]}
          />
        }
      >
        <View className="mb-10 flex-row justify-between items-end">
          <View>
            <Text className="text-[10px] font-black uppercase tracking-[0.5em] text-[var(--text-secondary)] mb-2 italic text-left">Strategic Maps</Text>
            <Text className="text-4xl font-black text-[var(--text-primary)] italic uppercase tracking-tighter text-left">
              Roadmaps
            </Text>
          </View>
          <TouchableOpacity 
            onPress={() => setCreationVisible(true)}
            className="w-14 h-14 bg-[var(--accent-color)] rounded-2xl items-center justify-center shadow-lg shadow-[var(--accent-color)]/20"
          >
            <Ionicons name="add" size={32} color={colors.primary} />
          </TouchableOpacity>
        </View>

        {loading && !isSyncing ? (
          <ActivityIndicator size="large" color="#0ea5e9" className="py-20" />
        ) : roadmaps.length === 0 ? (
          <View className="py-32 items-center justify-center border-2 border-dashed border-[var(--border-color)] rounded-[3rem] bg-[var(--bg-secondary)]/10">
            <Ionicons name="map-outline" size={48} color={colors.textSecondary} style={{ opacity: 0.2 }} />
            <Text className="text-sm font-black text-[var(--text-secondary)] uppercase tracking-widest italic mt-4 text-center">No active roadmaps found{'\n'}Architect your journey</Text>
            <TouchableOpacity 
              onPress={() => setCreationVisible(true)}
              className="mt-8 px-8 py-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl"
            >
              <Text className="text-[10px] font-black text-sky-500 uppercase tracking-widest">Initialize New Roadmap</Text>
            </TouchableOpacity>
          </View>
        ) : (
          roadmaps.map(t => <RoadmapCard key={t.id} track={t} />)
        )}

        <View className="mt-10 p-10 bg-sky-500/5 border border-sky-500/20 rounded-[3.5rem] text-left">
          <View className="flex-row items-center gap-3 mb-4">
            <View className="p-2 bg-sky-500/20 rounded-lg">
              <Ionicons name="navigate" size={14} color="#0ea5e9" />
            </View>
            <Text className="text-xs font-black text-sky-500 uppercase tracking-[0.2em] italic">Navigation Protocol</Text>
          </View>
          <Text className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest leading-relaxed opacity-60 italic text-left">
            Day-wise temporal sequences are deconstructed and synchronized across the neural network for maximum execution efficiency.
          </Text>
        </View>
      </ScrollView>

      <RoadmapInitModal 
        isVisible={creationVisible} 
        onClose={() => setCreationVisible(false)} 
        onRefresh={onRefresh}
      />
    </View>
  );
}
