...
- **Internal Packages**: All applications in the `apps/` directory import shared packages such as `@planner/db` for database access, `@planner/domain` for business logic, and `@planner/api` for input validation.
- **Consistency**: By using the shared configuration from `@planner/config`, all applications maintain a consistent navigation structure and system-wide settings.
- **Data Sharing**: All applications interact with the same backend database via the shared `@planner/db` package (either directly or via an API), ensuring that user data is synchronized across all platforms.
