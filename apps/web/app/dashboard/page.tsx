...
import { dashboard } from "@domain"; // ✅ Use the optimized domain service

export default async function DashboardHome() {
  const user = await getServerUser();
  if (!user) redirect("/login");

  // ✅ 1. Fetch Real Data (Optimized via Promise.all in domain)
  const data = await dashboard.getDashboardStats(user.id);

  if (!data) {
    return <div className="p-8 text-center text-rose-500">Failed to load dashboard data.</div>;
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8 fade-in">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{data.greeting}, {user.name}</h1>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-4xl font-bold">
                {Math.floor(focusMinutes / 60)}<span className="text-lg opacity-60">h</span> {focusMinutes % 60}<span className="text-lg opacity-60">m</span>
              </div>
            </div>
              ...
              <>
                ...
                <div className="w-full bg-slate-100 h-2 rounded-full mt-4 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.max(0, Math.min(100, data.activePlan.progress))}%` }}
                  />
                </div>
