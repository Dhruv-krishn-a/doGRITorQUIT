...
// ✅ Ensuring this is exported and correct
...
export default function PlanDetailClient({ initialPlan }: PlanDetailClientProps) {
  ...
  // ... (Keep existing toggleSubtask logic)
  ...
  // ... (Keep existing calculation logic: progress, timeline, etc.)
    ...
    <div className="p-6 max-w-5xl mx-auto fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="text-2xl font-bold text-slate-800 mt-1">{Math.floor(totalEstMinutes / 60)}h {totalEstMinutes % 60}m</div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <div className="text-xs text-slate-500 font-bold uppercase tracking-wider">Actual Time</div>
          <div className="text-2xl font-bold text-emerald-600 mt-1">{Math.floor(totalSpentMinutes / 60)}h {totalSpentMinutes % 60}m</div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <div className="text-xs text-slate-500 font-bold uppercase tracking-wider">Tasks Done</div>
          <div className="text-2xl font-bold text-slate-800 mt-1">{completedTasks} <span className="text-sm text-slate-400 font-normal">/ {totalTasks}</span></div>
        </div>
      </div>
        ...
        {sortedDates.map((dateKey) => {
          ...
          return (
            <div key={dateKey} className="relative pl-8">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">{displayDate}</h3>
                  ...
                  return (
                    <div key={task.id} className={`bg-white border rounded-xl p-5 transition-all ${isTaskCompleted ? "opacity-60 bg-gray-50 border-gray-100" : "shadow-sm hover:shadow-md border-slate-200"}`}>
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className={`font-bold text-lg ${isTaskCompleted ? "line-through text-slate-400" : "text-slate-800"}`}>{task.title}</h4>
                          <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                            {task.estimatedMinutes && <div className="flex items-center gap-1"><Clock size={12} /><span>Est: {task.estimatedMinutes}m</span></div>}
                            {(task.timeSpentMinutes || 0) > 0 && <div className="flex items-center gap-1 text-emerald-600 font-medium"><Clock size={12} /><span>Spent: {task.timeSpentMinutes}m</span></div>}
                          </div>
                        </div>
                        {task.priority && <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${task.priority === 'High' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500'}`}>{task.priority}</span>}
                      </div>
                      {task.description && <p className="text-sm text-slate-600 mt-2 whitespace-pre-line">{task.description}</p>}
                      {task.subtasks && task.subtasks.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-slate-100 space-y-2">
                          {task.subtasks.map(st => (
                            <div key={st.id} className="flex items-center gap-3 cursor-pointer group select-none" onClick={() => toggleSubtask(task.id, st.id, !st.completed)}>
