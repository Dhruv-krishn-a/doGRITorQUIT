  ...
  return (
    <>
      ...
      >
        <div className="transform-gpu w-full px-6 md:px-10 h-20 flex items-center justify-between">
          ...
          <div className="transform-gpu flex items-center gap-3 md:gap-4">
            ...
            {/* Mobile Menu Trigger */}
            <button onClick={() => setMobileMenuOpen(true)} className="transform-gpu md:hidden p-2.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl transition-all active:scale-95">
               <Menu size={20} />
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileMenuOpen(false)} className="transform-gpu fixed inset-0 bg-[var(--bg-primary)]/60 backdrop-blur-sm z-1200 md:hidden" />
            <motion.div 
              initial={{ x: "100%" }} 
              animate={{ x: 0 }} 
              exit={{ x: "100%" }} 
...
function DropdownItem({ href, icon, label, onClick }: { href: string; icon: React.ReactNode; label: string; onClick: () => void }) {
    return (
        <Link href={href} onClick={onClick} className="transform-gpu flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] rounded-xl hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)] transition-all group">
            <span className="transform-gpu text-[var(--text-secondary)] group-hover:text-[var(--accent-color)] transition-colors">{icon}</span>
            {label}
        </Link>
    );
}
