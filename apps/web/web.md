...
- **Internal Packages**: The web app imports shared packages such as `@gritorquit/db` for data access, `@gritorquit/domain` for business logic, and `@gritorquit/api` for data validation.
- **Authentication**: Uses `lib/auth.ts` and Supabase SSR to manage user sessions and protect routes.
- **Database Access**: Direct database interaction via Prisma is typically handled in server components or server actions using the shared `@gritorquit/db` package.
- **Consistency**: By using the `siteNav` configuration from `@gritorquit/config`, the web application's navigation is always in sync with other applications in the monorepo.
