...
export default function SubscriptionPanel() {
  ...
  if (loading) {
    return (
      <div className="transform-gpu flex h-48 items-center justify-center text-slate-400">
        <Loader2 className="transform-gpu animate-spin mr-2" /> Loading plan details...
      </div>
    );
  }
  ...
  return (
    <div className="transform-gpu space-y-8">
      ...
      <div className="transform-gpu bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h2 className="transform-gpu text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 flex items-center gap-2">
          <Shield size={14} /> Current Status
        </h2>
        {subscription ? (
          <div className="transform-gpu flex items-center justify-between">
            <div>
              <div className="transform-gpu text-2xl font-bold text-slate-900 capitalize flex items-center gap-2">
                {subscription.status} Plan
                <span className="transform-gpu bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded-full border border-emerald-200 uppercase font-bold tracking-wide">
                  Active
                </span>
              </div>
              <p className="transform-gpu text-slate-500 text-sm mt-1">
                Renews on {subscription.currentPeriodEnd ? new Date(subscription.currentPeriodEnd).toLocaleDateString() : "N/A"}
              </p>
            </div>
            <button disabled className="transform-gpu text-slate-400 text-sm font-medium border border-slate-200 px-4 py-2 rounded-lg bg-slate-50 cursor-not-allowed">
              Manage
            </button>
          </div>
        ) : (
          <div className="transform-gpu flex items-center justify-between">
            <div>
              <div className="transform-gpu text-xl font-bold text-slate-700">Free Tier</div>
              <p className="transform-gpu text-slate-500 text-sm">Upgrade to unlock all features.</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Plans Grid */}
      <div>
        <h3 className="transform-gpu text-lg font-bold text-slate-900 mb-4">Available Plans</h3>
        <div className="transform-gpu grid grid-cols-1 md:grid-cols-2 gap-4">
          {products.map((p) => {
            ...
            return (
              <div 
                key={p.key} 
                className="transform-gpu group border border-slate-200 hover:border-blue-400 hover:shadow-md transition-all rounded-xl p-6 bg-white flex flex-col"
              >
                <div className="transform-gpu flex justify-between items-start mb-4">
                  <div>
                    <h4 className="transform-gpu font-bold text-lg text-slate-900">{p.name}</h4>
                    <p className="transform-gpu text-slate-500 text-sm">{p.description || "Premium features included"}</p>
                  </div>
                  <div className="transform-gpu bg-blue-50 text-blue-700 p-2 rounded-lg">
                    <CreditCard size={20} />
                  </div>
                </div>
                
                <div className="transform-gpu mt-auto pt-4 border-t border-slate-100">
                  <div className="transform-gpu flex items-baseline gap-1 mb-4">
                    <span className="transform-gpu text-3xl font-bold text-slate-900">₹{p.price}</span>
                    <span className="transform-gpu text-slate-500 text-sm font-medium">/{p.currency.toLowerCase()}</span>
                  </div>
                  
                  <button 
                    disabled={processing || isCurrent}
                    onClick={() => buy(p.key)}
                    className="transform-gpu w-full py-2.5 rounded-lg bg-slate-900 text-white font-medium hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
                  >
                    {processing ? <Loader2 size={16} className="transform-gpu animate-spin" /> : null}
