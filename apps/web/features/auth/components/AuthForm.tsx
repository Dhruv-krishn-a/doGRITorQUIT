...
export default function AuthForm() {
  ...
  return (
    <div className="transform-gpu flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="transform-gpu w-full max-w-md space-y-8 rounded-2xl bg-white p-8 shadow-xl border border-slate-100">
        ...
        <form className="transform-gpu mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            />
            {isLogin && (
              <div className="transform-gpu mt-1 flex justify-end">
                <button type="button" onClick={() => router.push("/forgot-password")} className="transform-gpu text-xs font-medium text-indigo-600 hover:text-indigo-500">
                  Forgot password?
                </button>
              </div>
            )}
          ...
          >
            {loading ? <Loader2 className="transform-gpu animate-spin h-5 w-5" /> : (isLogin ? "Sign In" : "Sign Up")}
          </button>
        </form>

        <div className="transform-gpu relative">
          <div className="transform-gpu absolute inset-0 flex items-center"><div className="transform-gpu w-full border-t border-slate-200" /></div>
          <div className="transform-gpu relative flex justify-center text-sm"><span className="transform-gpu bg-white px-2 text-slate-500">Or continue with</span></div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="transform-gpu flex w-full items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition-all hover:bg-slate-50 hover:border-slate-300 disabled:opacity-50"
        >
          <svg className="transform-gpu h-5 w-5" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
          Google
        </button>

        <p className="transform-gpu text-center text-sm text-slate-600">
          {isLogin ? "Don't have an account?" : "Already have an account?"}{" "}
          <button onClick={() => setIsLogin(!isLogin)} className="transform-gpu font-bold text-indigo-600 hover:underline">
