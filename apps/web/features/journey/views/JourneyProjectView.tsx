...
import { ArrowLeft, Plus, RefreshCw, ShieldCheck, Target, Loader2, CheckCircle2, Circle, Sparkles, Trash2, Lock, RotateCcw, Layers, Users, Cpu, ChevronRight, Zap, AlertTriangle } from "lucide-react";
...
export function JourneyProjectView({ initialProject }: { initialProject: GithubProject & { features: GithubFeature[], requirementsNote?: any, userFlowNote?: any, systemFlowNote?: any, iterations?: any[] } }) {
  const router = useRouter();
  const [project, setProject] = useState(initialProject);
  
  const [viewMode, setViewMode] = useState<ViewMode>(project.projectStage === "EXECUTION" ? "PHASE_DASHBOARD" : "BLUEPRINT");
  const [blueprintType, setBlueprintType] = useState<BlueprintType>(
    project.projectStage === "USER_FLOW" ? "USER_FLOW" :
    project.projectStage === "SYSTEM_FLOW" ? "SYSTEM_FLOW" : "REQUIREMENTS"
  );
  ...
  const STAGES = useMemo(() => getStagesForMethodology(project.methodology), [project.methodology]);
  
  const [activePhaseId, setActivePhaseId] = useState<string | null>(activeIteration?.sdlcPhaseId || (STAGES.length > 0 ? STAGES[0].id : null));
  ...
  const handleUpdateProject = async (updates: Partial<GithubProject>) => {
    try {
      const res = await fetch(`/api/github-projects/${project.id}`, {
        method: "PATCH",
  ...
  const handleSaveNote = async (title: string, content: any) => {
    ...
    try {
      await fetch(`/api/notes/${noteId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, content }) });
  ...
  const handleSealBlueprint = async () => {
    ...
    setIsSealing(true);
    try {
      const res = await fetch(`/api/github-projects/${project.id}/notes/${noteId}/seal`, { method: "POST" });
  ...
  const handleAdvanceProjectStage = async () => {
    ...
    try {
      if (project.projectStage === "REQUIREMENTS") {
        const res = await fetch(`/api/github-projects/${project.id}/blueprint/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "USER_FLOW" }) });
        ...
        const res = await fetch(`/api/github-projects/${project.id}/blueprint/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "SYSTEM_FLOW" }) });
        const note = await res.json();
        ...
        const res = await fetch(`/api/github-projects/${project.id}/iterations`, { 
          method: "POST", 
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: "V1.0", methodology: project.methodology, sdlcPhaseId: STAGES[0].id }) 
        });
        const iteration = await res.json();
        
        const strategyRes = await fetch(`/api/github-projects/${project.id}/iterations/${iteration.id}/tasks/generate`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ methodology: project.methodology })
        });
        
        if (!strategyRes.ok) {
           throw new Error("Failed to generate master plan");
        }
        const epics = await strategyRes.json();
  ...
  const handleGeneratePhaseTasks = async () => {
    if (!activeIteration || !activePhaseId) return;
    setIsGenerating(true);
    try {
      const res = await fetch(`/api/github-projects/${project.id}/iterations/${activeIteration.id}/tasks/generate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phaseId: activePhaseId })
      });
      const newFeatures = await res.json();
      setProject(prev => ({ ...prev, features: [...prev.features, ...newFeatures] }));
  ...
  const handleCreateFeature = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !activePhaseId || !activeIteration) return;
    setIsCreatingFeature(true);
    try {
      const res = await fetch(`/api/github-projects/${project.id}/features`, { 
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
        body: JSON.stringify({ title, sdlcPhaseId: activePhaseId, iterationId: activeIteration.id, parentId: activeEpicId }) 
      });
  ...
  const handleUpdateFeatureStatus = async (featureId: string, status: string) => {
    try {
      const res = await fetch(`/api/github-projects/${project.id}/features/${featureId}/status`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) });
      const updated = await res.json();
      setProject(prev => ({ ...prev, features: prev.features.map(f => f.id === featureId ? updated : f) }));
    } catch (err) {
  ...
  const handleDeleteFeature = async (id: string) => {
    setIsDeleting(true);
    try {
      await fetch(`/api/github-projects/${project.id}/features/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="relative w-full min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] selection:bg-[var(--accent-color)]/30 font-sans flex flex-col overflow-hidden">
      ...
      <header className="relative z-10 px-6 md:px-8 py-3.5 border-b border-[var(--border-color)] bg-[var(--bg-card)]/80 backdrop-blur-xl flex items-center justify-between shrink-0">
        <div className="flex items-center gap-5">
          <button onClick={() => router.push('/dashboard/project-tracker')} className="p-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg transition-all shadow-sm active:scale-95"><ArrowLeft size={16} /></button>
          ...
          <div className="p-6 border-b border-[var(--border-color)] space-y-6">
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => { setBlueprintType("REQUIREMENTS"); setViewMode("BLUEPRINT"); }} className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${viewMode === "BLUEPRINT" && blueprintType === "REQUIREMENTS" ? "bg-amber-500/10 border-amber-500/40 text-amber-500" : "bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:border-amber-500/20"}`}>
                  <div className="flex items-center gap-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => { if (project.userFlowNoteId) { setBlueprintType("USER_FLOW"); setViewMode("BLUEPRINT"); } }} className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition-all ${viewMode === "BLUEPRINT" && blueprintType === "USER_FLOW" ? "bg-amber-500/10 border-amber-500/40 text-amber-500" : "bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)]"} ${!project.userFlowNoteId ? "opacity-30 cursor-not-allowed" : "hover:border-amber-500/20"}`}>
                  ...
                  <button onClick={() => { if (project.systemFlowNoteId) { setBlueprintType("SYSTEM_FLOW"); setViewMode("BLUEPRINT"); } }} className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition-all ${viewMode === "BLUEPRINT" && blueprintType === "SYSTEM_FLOW" ? "bg-amber-500/10 border-amber-500/40 text-amber-500" : "bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)]"} ${!project.systemFlowNoteId ? "opacity-30 cursor-not-allowed" : "hover:border-amber-500/20"}`}>
                ...
                <button onClick={() => setViewMode("PHASE_DASHBOARD")} className={`w-full flex items-center justify-center gap-3 p-3 rounded-xl border transition-all ${viewMode === "PHASE_DASHBOARD" && project.projectStage === "METHODOLOGY" ? "bg-amber-500/10 border-amber-500/40 text-amber-500 shadow-lg shadow-amber-500/10" : "bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:border-amber-500/20"} ${!project.systemFlowVerified ? "opacity-30 cursor-not-allowed" : ""} ${project.projectStage === "METHODOLOGY" && !project.methodology ? "animate-pulse border-amber-500/50" : ""}`}>
              ...
              <div className="max-w-5xl mx-auto space-y-8 pb-32">
                ...
                <div className="sticky bottom-0 mt-8 bg-[var(--bg-card)]/90 backdrop-blur-xl border border-[var(--border-color)] rounded-3xl p-6 shadow-2xl flex items-center justify-between gap-6 z-20">
                  ...
                  <button onClick={() => { 
                     ...
                     if (curIdx < STAGES.length - 1) setActivePhaseId(STAGES[curIdx + 1].id);
