...
import { Plus, Github, ArrowRight, Loader2, Server, RefreshCw, ShieldCheck, Target, Trash2, X, Sparkles } from "lucide-react";
import { GithubProject } from "@gritorquit/domain/github-projects/types";
...
const getLifecycleIcon = (lifecycle: string, stage: string) => {
  if (stage === 'REQUIREMENTS') return Sparkles;
  switch (lifecycle) {
...
const getLifecycleColor = (lifecycle: string, stage: string) => {
  if (stage === 'REQUIREMENTS') return "text-amber-500 bg-amber-500/10 border-amber-500/20";
  switch (lifecycle) {
...
export function JourneyHubView() {
  ...
  const handleProjectCreated = (projectId: string) => {
    ...
    router.push(`/dashboard/project-tracker/${projectId}`);
  };

  const handleDeleteProject = async (projectId: string) => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/github-projects/${projectId}`, {
        method: "DELETE"
      });
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] bg-[var(--bg-primary)] w-full">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-[var(--text-secondary)] font-bold uppercase tracking-widest text-xs">Booting OS...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] selection:bg-[var(--accent-color)]/30 font-sans overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto space-y-12 pt-10 pb-24 px-6 md:px-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        {projects.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[40vh] border-2 border-dashed border-[var(--border-color)] rounded-[3.5rem] bg-[var(--bg-card)]/30 backdrop-blur-sm p-10 text-center">
             ...
             <h3 className="text-xl font-black text-[var(--text-primary)] uppercase tracking-tighter italic mb-2">No Projects Found</h3>
             <p className="text-sm font-bold text-[var(--text-secondary)] uppercase tracking-widest max-w-md mx-auto mb-8 leading-relaxed opacity-60">
               Start your first project to begin tracking your progress.
             </p>
             <button 
                onClick={() => setIsModalOpen(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project: any) => {
              const isPlanning = project.requirementsNoteId && project.features.length === 0;
              const displayLifecycle = isPlanning ? "PLANNING" : project.lifecycle;
              const Icon = getLifecycleIcon(project.lifecycle, isPlanning ? 'REQUIREMENTS' : 'DONE');
              const colorClass = getLifecycleColor(project.lifecycle, isPlanning ? 'REQUIREMENTS' : 'DONE');
              
              return (
                <div
                  key={project.id}
                  onClick={() => router.push(`/dashboard/project-tracker/${project.id}`)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      router.push(`/dashboard/project-tracker/${project.id}`);
                    }
                  }}
                  ...
                  <div className="mt-auto pt-6 border-t border-[var(--border-color)] flex items-center justify-between w-full">
                    {project.githubRepo ? (
                      <div className="flex items-center gap-2 text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">
                        <Github size={12} /> {project.githubRepo}
                      </div>
                    ) : (
                      <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest opacity-40">No Repo Linked</span>
                    )}
        ...
        {projectToDelete && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
            <motion.div 
              ...
              className="w-full max-w-md bg-[var(--bg-primary)] border border-rose-500/20 rounded-[2.5rem] p-10 text-center shadow-2xl"
            >
               ...
               <h3 className="text-2xl font-black uppercase tracking-tighter text-[var(--text-primary)] italic mb-3">Delete Project?</h3>
               <p className="text-[11px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.2em] leading-relaxed mb-10 opacity-60">
                 This will permanently delete the project, all its features, linked branches, and requirements notes. This action cannot be reversed.
               </p>
               <div className="flex items-center gap-4">
                 <button 
                   onClick={() => setProjectToDelete(null)}
                   className="flex-1 py-4 border border-[var(--border-color)] rounded-2xl text-[10px] font-black uppercase tracking-widest text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-all"
                 >
                   Keep Project
                 </button>
                 <button 
                   ...
                   disabled={isDeleting}
                   className="flex-1 py-4 bg-rose-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:opacity-90 disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-lg shadow-rose-500/20"
                 >
