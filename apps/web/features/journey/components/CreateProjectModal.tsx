...
export function CreateProjectModal({ isOpen, onClose, onProjectCreated }: CreateProjectModalProps) {
  ...
  const content = (
    <AnimatePresence>
      {isOpen && (
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 md:p-6 text-left">
            {/* Dual Layer Backdrop for persistent high-quality blur */}
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="relative z-10 w-full max-w-2xl bg-[var(--bg-primary)] border border-[var(--border-color)] shadow-2xl rounded-[3rem] overflow-hidden flex flex-col max-h-[90vh] transform-gpu antialiased"
            >
                {/* Animated Background Gradients */}
                <div className="transform-gpu absolute top-[-10%] left-[-10%] w-[30vw] h-[30vw] bg-[var(--accent-color)]/5 rounded-full blur-[80px] mix-blend-screen pointer-events-none" />
                <div className="transform-gpu absolute bottom-[-10%] right-[-10%] w-[40vw] h-[40vw] bg-indigo-500/5 rounded-full blur-[100px] mix-blend-screen pointer-events-none" />
                ...
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
                        {mode === "SELECT" ? (
                            ...
                            <form id="create-project-form" onSubmit={handleSubmit} className="space-y-10">
                            <div className="space-y-6">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] focus:border-[var(--accent-color)] text-[var(--text-primary)] px-6 py-5 rounded-2xl outline-none transition-all placeholder:text-[var(--text-secondary)]/30 font-black italic tracking-tight uppercase"
                                    autoFocus
                                />
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        className="w-full flex items-center justify-between bg-[var(--bg-secondary)] border border-[var(--border-color)] focus:border-[var(--accent-color)] text-[var(--text-primary)] px-6 py-5 rounded-2xl outline-none transition-all font-black italic tracking-tight"
                                        >
                            ...
                            {mode === "EXISTING" && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <button
                                        ...
                                        className={`flex flex-col text-left p-6 rounded-[2rem] border transition-all ${lifecycle === option.id ? `${option.border} ${option.bg} shadow-lg shadow-${option.color.replace('text-', '')}/10` : 'border-[var(--border-color)] bg-[var(--bg-secondary)]/50 hover:bg-[var(--bg-secondary)] hover:border-[var(--accent-color)]/30'}`}
                                    >
                    ...
                    {mode !== "SELECT" && (
                        <div className="p-8 md:p-10 border-t border-[var(--border-color)] bg-[var(--bg-secondary)]/30 flex justify-between items-center gap-4">
                            <button onClick={() => setMode("SELECT")} className="px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all italic bg-[var(--bg-secondary)] border border-[var(--border-color)] active:scale-95">← Back</button>
                            <button 
                            type="submit"
