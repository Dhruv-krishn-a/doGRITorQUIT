...
export function ProjectDetailView() {
  ...
  const { fetchTrack, activeTrack, loading, openModal, moveUnit, planToday, updateTrack, deleteUnit, addUnit, updateUnit, logProgress } = useStudy();
  ...
  const handleAction = (type: 'SESSION' | 'TIMER' | 'COMPLETE', unit: Unit) => {
    if (type === 'SESSION') {
      router.push(`/dashboard/study/project/${trackId}/${unit.id}`);
    } else if (type === 'TIMER') {
      router.push(`/dashboard/study/project/${trackId}/${unit.id}?layout=FULL_NOTES&autostart=true`);
    } else if (type === 'COMPLETE') {
      openModal('SESSION', unit, 'LOGS');
    }
  };

  const handleDragEnd = async (result: { draggableId: string; destination: { droppableId: string; index: number } | null }) => {
  ...
  return (
    <div className="transform-gpu flex flex-col w-full h-full flex-1 min-h-[calc(100vh-80px)] bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-hidden font-sans selection:bg-[var(--accent-color)]/30 relative rounded-tl-3xl">
      ...
      <ProjectHeader 
        {...sharedProps}
        onBack={() => router.push('/dashboard/study')}
        seconds={seconds}
