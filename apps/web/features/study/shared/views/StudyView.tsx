...
const VideoPanel = ({
  ...
  return (
    <div className={`h-full flex flex-col bg-white overflow-hidden border shadow-xl relative ${isDeepWork ? "rounded-none border-none" : "rounded-[2.5rem] border-rose-100/50"}`}>
    ...
    return (
      <div className={`h-full flex flex-col bg-white border shadow-xl overflow-hidden ${isDeepWork ? "rounded-none border-none" : "rounded-[2.5rem] border-rose-100"}`}>
        <div className="flex-1 p-6 md:p-8 flex flex-col h-full overflow-hidden">
          <div className="transform-gpu flex gap-2 mb-6">
            {(['NOTES', 'QUESTIONS'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setCurrentTab(tab)}
                className={`px-6 py-2.5 rounded-full text-[10px] font-bold tracking-widest transition-all ${currentTab === tab ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
              >
                {tab}
              </button>
            ))}
          ...
          <div className="flex-1 min-h-0">
            <Suspense fallback={<div className="h-full w-full flex items-center justify-center"><Loader2 className="animate-spin text-rose-500" size={24} /></div>}>
              <LazyNoteEditor
    ...
    <div className={`h-full flex flex-col bg-white border shadow-xl overflow-hidden ${isDeepWork ? "rounded-none border-none" : "rounded-[2.5rem] border-rose-100"}`}>
      <div className="transform-gpu flex-1 p-8 flex flex-col h-full overflow-hidden">
          ...
          {Array.isArray(questions) && questions.map((note, idx) => (
            <div key={note.id || idx} className="transform-gpu bg-amber-50 border border-amber-100 p-4 rounded-2xl group hover:border-amber-200 transition-all">
              <div className="transform-gpu flex justify-between items-start mb-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-200/50 text-amber-700 uppercase tracking-widest">
                ...
                <button 
                  ...
                  className="transform-gpu flex items-center gap-1.5 text-[10px] font-bold text-rose-500 bg-white px-3 py-1.5 rounded-lg hover:bg-rose-50 shadow-sm border border-rose-50 transition-colors"
                >
                  <Play size={10} fill="currentColor" />
                  {Math.floor(note.timestampSeconds / 60)}:{(Math.floor(note.timestampSeconds) % 60).toString().padStart(2, '0')}
                </button>
              </div>
              <p className="transform-gpu text-sm text-slate-800 whitespace-pre-wrap font-medium">{note.content}</p>
            </div>
          ))}
          {questions.length === 0 && (
        ...
        <div className="transform-gpu shrink-0 relative mt-auto">
          <textarea
            className="transform-gpu w-full bg-slate-50 border border-slate-200 focus:border-rose-300 focus:ring-2 focus:ring-rose-100 rounded-3xl p-6 text-sm resize-none transition-all placeholder:text-slate-400 font-medium"
            rows={3}
          ...
          />
          <div className="transform-gpu absolute bottom-6 right-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Press Enter ↵
          </div>
        </div>
      </div>
...
export function StudyView() {
  ...
  return (
    <div className={`transform-gpu min-h-screen ${isDeepWork ? "bg-black" : "bg-[#fdfbfb]"} transition-colors duration-500`}>
      <header className={`transform-gpu border-b px-8 py-5 flex items-center justify-between sticky top-0 z-50 transition-all duration-300 ${isDeepWork ? "bg-black border-white/5 opacity-0 hover:opacity-100 h-1" : "bg-white/80 backdrop-blur-xl border-rose-50"}`}>
        <div className="transform-gpu flex items-center gap-6">
          <button onClick={() => router.back()} className="transform-gpu p-3 hover:bg-rose-50 text-slate-400 hover:text-rose-500 rounded-2xl transition-all active:scale-90">
            <ArrowLeft size={20} />
          </button>
          <div className="transform-gpu h-8 w-px bg-rose-100/50" />
          <div className="transform-gpu flex items-center gap-4">
            <div className="transform-gpu w-10 h-10 rounded-xl bg-rose-500 flex items-center justify-center text-white shadow-lg shadow-rose-200">
              <Brain size={20} />
            </div>
            <div>
              <p className="transform-gpu text-[10px] font-bold text-rose-500 uppercase tracking-widest leading-none mb-1">Studying Vector</p>
              <h2 className="transform-gpu text-sm font-bold text-slate-800 tracking-tight leading-none">{unit?.title}</h2>
            </div>
          </div>
        </div>
        ...
        <div className="transform-gpu flex items-center gap-3">
          <button 
            ...
            className={`flex items-center gap-3 px-6 py-3 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all ${isDeepWork ? "bg-rose-600 text-white shadow-lg shadow-rose-900/40" : "bg-white border border-rose-100 text-rose-500 hover:bg-rose-50"}`}
          >
            <Zap size={14} className={isDeepWork ? "animate-pulse" : ""} />
            {isDeepWork ? "Deep Work Active" : "Enable Deep Work"}
          </button>
          
          <button 
             onClick={() => openModal('SESSION', unit, 'LOGS')}
             className="transform-gpu px-8 py-3 bg-slate-900 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl shadow-slate-200 active:scale-95"
          >
            Complete Session
          </button>
        </div>
      ...
      <main className={`transform-gpu transition-all duration-500 ${isDeepWork ? "p-0 h-[calc(100vh-4px)]" : "p-8 md:p-10 h-[calc(100vh-80px)]"}`}>
