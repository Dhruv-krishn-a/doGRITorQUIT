...
interface RoadmapTrackerViewProps {
  initialPlans?: Plan[];
  isLimitReached?: boolean;
  maxPlans?: number;
}

export function RoadmapTrackerView({ initialPlans = [], isLimitReached = false, maxPlans = 1 }: RoadmapTrackerViewProps) {
  ...
  if (loading) return (
    <div className="transform-gpu flex items-center justify-center min-h-[60vh] bg-[var(--bg-primary)] w-full">
      <div className="transform-gpu flex flex-col items-center gap-4">
        ...
        <div className="transform-gpu text-[var(--text-secondary)] font-bold uppercase tracking-widest text-xs">Syncing Roadmaps...</div>
      </div>
    </div>
  );
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto space-y-12 pt-10 pb-24 px-6 md:px-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <header className="flex flex-col md:flex-row justify-between md:items-center gap-8 pb-8 border-b border-[var(--border-color)]">
          <div className="space-y-2">
            ...
            <p className="text-[var(--text-secondary)] font-bold uppercase tracking-[0.2em] text-[10px] flex items-center gap-2 text-left">
