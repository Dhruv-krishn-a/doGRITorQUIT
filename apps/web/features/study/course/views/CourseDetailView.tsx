...
export function CourseDetailView() {
  ...
  if (loading && !activeTrack) return (
    <div className="flex items-center justify-center min-h-[60vh] bg-[#0a0105] w-full">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-rose-300/50 font-black uppercase tracking-widest text-xs">Accessing Knowledge Base...</div>
      </div>
    </div>
  );
  ...
  if (!activeTrack) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 bg-[#0a0105] w-full">
      <div className="text-rose-400/50 font-black uppercase tracking-widest text-sm">Course link severed.</div>
      <button 
        onClick={() => router.push('/dashboard/study')} 
        className="px-8 py-4 bg-linear-to-r from-rose-600 to-pink-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-[0_0_15px_rgba(244,63,94,0.4)] hover:shadow-[0_0_25px_rgba(244,63,94,0.6)] border border-rose-400/50 transition-all active:scale-95"
      >
  ...
  return (
    <div className="flex-1 min-w-0 relative space-y-12 text-rose-100 pb-24 w-full px-4 md:px-8 font-sans bg-[#0a0105] min-h-screen">
      <div className="fixed top-0 right-0 w-96 h-96 bg-fuchsia-500/10 rounded-full blur-[120px] -z-10 pointer-events-none opacity-50" />
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-8 w-full">
          ...
          <div className="flex-1 min-w-0">
             <div className="flex items-center gap-4 mb-2">
               ...
               <span className="text-[10px] font-black bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/30 shadow-[0_0_10px_rgba(217,70,239,0.2)] px-3 py-1 rounded-lg uppercase tracking-widest shrink-0 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-4 bg-[#14030b] border border-fuchsia-900/40 p-2 rounded-2xl shadow-xl">
             <div className="px-4 py-2 border-r border-fuchsia-900/20">
                <p className="text-[8px] font-black uppercase text-fuchsia-400/40 tracking-widest">Focus Duration</p>
                <p className="text-xl font-black text-rose-50 font-mono tracking-tighter">{formatTime(seconds)}</p>
             </div>
          ...
          <div className="flex flex-wrap gap-4 shrink-0">
            ...
            <button onClick={async () => { await fetchTrack(track.id); openModal('DELETE'); }} className="p-5 bg-[#14030b] border border-red-900/50 rounded-3xl text-red-400/60 hover:text-red-400 hover:bg-red-950/30 hover:border-red-500/50 hover:shadow-[0_0_15px_rgba(239,68,68,0.2)] transition-all active:scale-95"><Trash2 size={24} /></button>
          </div>
        </div>
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between border-b border-rose-900/40 pb-6">
          <div className="flex gap-10">
            <button 
              ...
              className={`text-xl font-black uppercase tracking-tight flex items-center gap-3 transition-all ${activeTab === 'BOARD' ? 'text-rose-50' : 'text-rose-400/30 hover:text-rose-400/60'}`}
            >
              <LayoutGrid size={20} className={activeTab === 'BOARD' ? 'text-fuchsia-500' : ''} /> Lesson Board
            </button>
            <button 
              onClick={() => setActiveTab('NOTES')}
              className={`text-xl font-black uppercase tracking-tight flex items-center gap-3 transition-all ${activeTab === 'NOTES' ? 'text-rose-50' : 'text-rose-400/30 hover:text-rose-400/60'}`}
            >
        ...
        <main className="animate-in fade-in slide-in-from-bottom-8 duration-1000 relative">
              ...
              <motion.div key="notes" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                 <div className="bg-[#14030b] border border-rose-900/40 rounded-[2.5rem] p-10 space-y-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-2xl font-black text-rose-50 uppercase tracking-tight">Curriculum Notes</h3>
                        <p className="text-[10px] font-bold text-rose-400/40 uppercase tracking-widest mt-1">Syllabus-wide synthesis and core concepts</p>
                      </div>
                      <button 
                        onClick={async () => {
                          setIsSaving(true);
                        ...
                        className="flex items-center gap-2 px-8 py-4 bg-linear-to-r from-fuchsia-600 to-purple-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:from-fuchsia-500 hover:to-purple-500 shadow-lg shadow-fuchsia-900/40"
                      >
